...
void GraphicsController::paintEvent(QPaintEvent *event)
{
	...
	if(displayData.highlightT)
	{
        ...
        SplineVector3D result = mainSpline->getPosition(displayData.highlightedT);
...
QPoint GraphicsController::convertPoint(const SplineVector3D &point)
{
	return QPoint(int(point.x()), int(point.y()));
}

SplineVector3D GraphicsController::convertPoint(const QPoint &point)
{
	return SplineVector3D(point.x(), point.y(),0);
}
...
void GraphicsController::createDistanceField(const QString &filename)
{
	...
	std::vector<SplineVector3D> colorList;
    for(size_t i = 0; i < mainSpline->getPoints().size(); i++)
	{
		colorList.push_back(SplineVector3D(
    ...
    SplineVector3D previousPoint = mainSpline->getPosition(0);

	painter.setPen(Qt::red);

	while(currentStep <= limit)
	{
        SplineVector3D currentPoint = mainSpline->getPosition(currentStep);
...
int GraphicsController::pickVertex(const QPoint &screenPoint)
{
	...
	SplineVector3D convertedPoint = convertPoint(screenPoint);
    for(size_t i = 0; i < points.size(); i++)
	{
		SplineVector3D position = points[i];
...
SplineVector3D GraphicsController::getColor(float t) const
{
	SplineVector3D value = colorSpline->getPosition(t);
...
void GraphicsController::drawSplineSegment(QPainter &painter, const std::shared_ptr<Spline> &s, double beginT, double endT, double thresholdAngle)
{
    ...
    SplineVector3D beginNormalizedTangent = beginData.tangent.normalized();
    SplineVector3D endNormalizedTangent = endData.tangent.normalized();

    //compute the angle between the two tangents
    double cosAngle = SplineVector3D::dotProduct(beginNormalizedTangent, endNormalizedTangent);
    ...
    if(cosAngle < thresholdAngle && (endT - beginT) > minDelta)
    {
        ...
        double beginProjection = SplineVector3D::dotProduct(beginNormalizedTangent, beginData.curvature);
        double endProjection = SplineVector3D::dotProduct(endNormalizedTangent, endData.curvature);

        SplineVector3D beginRejection = beginData.curvature - beginNormalizedTangent * beginProjection;
        SplineVector3D endRejection = endData.curvature - endNormalizedTangent * endProjection;
...
void GraphicsController::drawPoints(QPainter &painter, const std::vector<SplineVector3D> &points)
{
    ...
    for(int i = 0; i < size; i++)
    {
        ...
        SplineVector3D position = points.at(i);
