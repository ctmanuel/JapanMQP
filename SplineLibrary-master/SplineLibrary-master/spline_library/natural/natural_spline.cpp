...
NaturalSpline::NaturalSpline(const std::vector<SplineVector3D> &points, bool includeEndpoints, double alpha)
{
    this->points = points;
    ...
    std::vector<SplineVector3D> deltaPoint(loop_limit);
    for(size_t i = 0; i < loop_limit; i++)
    {
        SplineVector3D displacement = points.at(i + 1) - points.at(i);
        deltaPoint[i] = displacement / upperDiagonal.at(i);
    }

    //create an array that stores 3 * (deltaPoint(i - 1) + deltaPoint(i))
    std::vector<SplineVector3D> inputVector(loop_limit - 1);
    for(size_t i = 1; i < loop_limit; i++)
    {
        SplineVector3D neighborDelta = 3 * (deltaPoint.at(i) - deltaPoint.at(i - 1));
    ...
    std::vector<SplineVector3D> curvatures = LinearAlgebra::solveSymmetricTridiagonal(diagonal, upperDiagonal, inputVector);

    //we didn't compute the first or last curvature, which will be 0
    curvatures.insert(curvatures.begin(), SplineVector3D());
    curvatures.push_back(SplineVector3D());
    ...
    for(int i = firstPoint; i < firstPoint + numSegments; i++) {
        ...
        SplineVector3D currentPoint = points.at(i);
        SplineVector3D nextPoint = points.at(i + 1);
        SplineVector3D currentCurvature = curvatures.at(i);
        SplineVector3D nextCurvature = curvatures.at(i + 1);
...
SplineVector3D NaturalSpline::getPosition(double x) const
{
...
const std::vector<SplineVector3D> &NaturalSpline::getPoints(void) const
{
    return points;
}
