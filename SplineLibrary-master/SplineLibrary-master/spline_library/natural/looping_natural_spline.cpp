...
LoopingNaturalSpline::LoopingNaturalSpline(const std::vector<SplineVector3D> &points, double alpha)
{
    this->points = points;

    std::unordered_map<int, double> indexToT_Raw;
    std::unordered_map<int, SplineVector3D> pointMap;
    ...
    std::vector<SplineVector3D> deltaPoint(size);
    for(size_t i = 0; i < size; i++)
    {
        SplineVector3D displacement = points.at((i + 1)%size) - points.at(i);
        deltaPoint[i] = displacement / upperDiagonal.at(i);
    }

    //create an array that stores 3 * (deltaPoint(i - 1) + deltaPoint(i))
    //when i = 0, wrap i - 1 back around to the end of the list
    std::vector<SplineVector3D> inputVector(size);
    for(size_t i = 0; i < size; i++)
    {
        SplineVector3D neighborDelta = 3 * (deltaPoint.at(i) - deltaPoint.at((i - 1 + size) % size));
        inputVector[i] = neighborDelta;
    }

    //solve the cyclic tridiagonal system to get the curvature at each point
    std::vector<SplineVector3D> curvatures = LinearAlgebra::solveCyclicSymmetricTridiagonal(diagonal, upperDiagonal, inputVector);
    ...
    for(int i = 0; i < numSegments; i++) {
        ...
        SplineVector3D nextPoint = points.at((i + 1)%size);
        SplineVector3D currentCurvature = curvatures.at(i);
        SplineVector3D nextCurvature = curvatures.at((i + 1)%size);
...
SplineVector3D LoopingNaturalSpline::getPosition(double x) const
{
